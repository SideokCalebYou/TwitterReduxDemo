...
class TwitterClient: BDBOAuth1SessionManager {
  ...
  func selectedUserTimeLine(screen_name: String, success: @escaping ([Tweet]) -> (), failure: @escaping (Error) -> ()){
   let getUserTimeLineUrl = "1.1/statuses/user_timeline.json?screen_name=\(screen_name)"
    get(getUserTimeLineUrl, parameters:nil, progress:nil, success:{(task:URLSessionDataTask, response:Any?) in
        let dictionaries = response as! [NSDictionary]
        let tweets = Tweet.tweetsWithArray(dictionaries: dictionaries)
        success(tweets)
    }, failure: {(task:URLSessionDataTask?, error:Error) in
  ...
  func showUser(screen_name: String, success: @escaping (User) -> (), failure: @escaping (Error) -> ()){
    let showUserUrl = "1.1/users/show.json?screen_name=\(screen_name)"
    get(showUserUrl, parameters:nil, progress: nil, success: {(task: URLSessionDataTask, response:Any?) in
    let userDictionary = response as! NSDictionary
      let user = User(dictionary: userDictionary)
      success(user)
    }, failure: {(task:URLSessionDataTask?, error:Error) in
  ...
  func getMentionTimeLine(success: @escaping ([Tweet]) -> (), failure: @escaping (Error) -> ()){
  ...
  func login(success: @escaping ()->(), failure: @escaping (Error)->()){
    ...
    TwitterClient.sharedInstance?.fetchRequestToken(withPath: "oauth/request_token", method: "GET", callbackURL: URL(string:"mytwitterreduxdemo://oauth"), scope: nil, success: { (requestToken : BDBOAuth1Credential?) in
